...
export default function CustomerJobsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<JobsResponse | Job[]>('/api/jobs');
      setJobs(Array.isArray(data) ? data : data.jobs || []);
    } catch (err: unknown) {
      setError((err as { message?: string })?.message || 'Could not load your jobs.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        title="My jobs"
        subtitle="Track service jobs from request to completion."
      />
      ...
      {loading ? (
        ...
        <div className={styles.emptyWrap}>
          <EmptyState
            icon="fa-briefcase"
            title="No jobs yet"
            description="Jobs appear here after a quote is accepted and scheduled with a provider."
        ...
        <div className={styles.cardGrid}>
          {jobs.map((job) => (
            <Link href={`/profile/jobs/${job.id}`} className={styles.requestCard} key={job.id}>
